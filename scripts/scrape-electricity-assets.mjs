/**
 * Download assets (images, CSS, JS) from electricitymonitorgh.com
 * 
 * Usage:
 *   node scripts/scrape-electricity-assets.mjs --contentDir content/electricity-monitor --concurrency 6
 */
...
function parseArgs(argv) {
  const args = {
    contentDir: "content/electricity-monitor",
    concurrency: DEFAULT_CONCURRENCY,
    timeoutMs: DEFAULT_TIMEOUT_MS,
    userAgent: "ACEP-Electricity-Asset-Scraper/1.0 (+https://acep.africa) - internal migration",
    allowedHosts: new Set(["electricitymonitorgh.com", "www.electricitymonitorgh.com"]),
  };
...
async function main() {
  ...
  console.log(`[electricity-assets] Candidate assets: ${assetUrls.length}`);
  ...
  async function worker() {
    while (true) {
      ...
      try {
        ...
        if ((n + 1) % 25 === 0) {
          console.log(`[electricity-assets] ${n + 1}/${assetUrls.length} downloaded`);
        }
      } catch (err) {
        manifest.assets.push({
          url,
        ...
        manifest.totals.errors++;
        console.error(`[electricity-assets] ERROR ${url}: ${String(err?.message || err)}`);
      }
    }
  }
  ...
  console.log(`\n[electricity-assets] Done!`);
  console.log(`[electricity-assets] Saved manifest: ${manifestPath}`);
  console.log(`[electricity-assets] Success: ${manifest.totals.successful}/${manifest.totals.assets}`);
  console.log(`[electricity-assets] Errors: ${manifest.totals.errors}/${manifest.totals.assets}`);
}

main().catch((e) => {
  console.error(`[electricity-assets] Fatal: ${e?.stack || e}`);
  process.exit(1);
});
