...
import { sendExhibitorConfirmationEmail } from "@/lib/event-mail";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const rl = await checkEventPublicRateLimit(`exh:${ip}:${slug}`);
  if (!rl.ok) {
  ...
  if (!event || event.status !== "PUBLISHED" || !event.publicExhibitorRegistration) {
  ...
  const website = parsed.data.website === "" ? undefined : parsed.data.website;

  const used = await prisma.eventExhibitorRegistration.count({
    where: { eventId: event.id, status: { in: ["PENDING_EMAIL", "CONFIRMED"] } },
  });
  if (event.maxExhibitorRegistrations != null && used >= event.maxExhibitorRegistrations) {
    return NextResponse.json({ ok: false, error: "exhibitor_full" }, { status: 400 });
  }

  const existing = await prisma.eventExhibitorRegistration.findUnique({
  ...
  if (existing?.status === "CANCELLED") {
    await prisma.eventExhibitorRegistration.delete({ where: { id: existing.id } });
  }
  ...
  const reg = await prisma.eventExhibitorRegistration.create({
    data: {
      eventId: event.id,
  ...
  const confirmUrl = `${base}/api/public/events/exhibitor-confirm?token=${encodeURIComponent(plain)}`;
  const mail = await sendExhibitorConfirmationEmail(email, event.title, confirmUrl);
